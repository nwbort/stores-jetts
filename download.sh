...
# Pretty-print and sort JSON by id if applicable
if [ "$EXTENSION" = ".json" ]; then
  # Create another temporary file for the processed version
  PRETTY_TEMP=$(mktemp)
  # Try to sort by id and pretty-print with jq
  if command -v jq &> /dev/null; then
    # Sort .items array by .id if it exists, otherwise just pretty-print
    if jq 'if .items then .items |= sort_by(.id) else . end' "$TEMP_FILE" > "$PRETTY_TEMP" 2>/dev/null; then
